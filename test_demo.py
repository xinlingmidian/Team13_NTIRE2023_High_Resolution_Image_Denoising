...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team13_DUNetformer import DUNetformer    
        name, data_range = f"{model_id:02}_baseline", 1.0
        model_path = os.path.join('model_zoo', 'team13_DUNetformer.pth')
        model = DUNetformer()
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    for i, (img_noisy, img_hr) in enumerate(data_path):
        ...
        # --------------------------------
        start.record()
        img_dn = forward(img_noisy, model, tile)# the batchsize must be 1
        end.record()
